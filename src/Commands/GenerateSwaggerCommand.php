...
PHP;
    ...
    private function buildSwaggerFileContent(string $className, string $tag, string $operations, array $securitySchemes, string $schemaBlock): string
    {
        ...
        $lines[] = '';
        $lines[] = '/**';
        $lines[] = ' * @OA\Tag(name="' . $tag . '")';
        $lines[] = ' *';
        $lines[] = ' * Note: This file contains OpenAPI annotations that work WITHOUT any external dependencies.';
        $lines[] = ' * ';
        $lines[] = ' * The @OA\ annotations here are processed by:';
        $lines[] = ' * - zircote/swagger-php (https://github.com/zircote/swagger-php) - optional';
        $lines[] = ' * - darkaonline/l5-swagger (wrapper for swagger-php) - optional';
        $lines[] = ' *';
        $lines[] = ' * You can use this file standalone OR integrate it with optional packages.';
        $lines[] = ' */';
